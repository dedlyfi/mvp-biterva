...
class Connection(Compat):
    ...
    def rewrite_query(self, query) -> str:
        ...
        elif self.type == SQLITE and "?" in query:
            # Replace ? with :p0, :p1, :p2... for SQLAlchemy text() compatibility
            parts = query.split("?")
            if len(parts) > 1:
                new_query = ""
    ...
    def rewrite_values(self, values: dict | tuple | list) -> dict | tuple | list:
        # strip html
        clean_regex = re.compile("<.*?>|&([a-z0-9]+|#[0-9]{1,6}|#x[0-9a-f]{1,6});")
        
        if isinstance(values, (tuple, list)):
            if self.type == SQLITE:
                # Convert tuple to dict params for SQLite named binding
                clean_values = {}
                for i, raw_value in enumerate(values):
                    val = raw_value
                    if isinstance(raw_value, str):
                        val = re.sub(clean_regex, "", raw_value)
                    elif isinstance(raw_value, datetime):
                        ts = raw_value.timestamp()
                        val = int(ts)
                    clean_values[f"p{i}"] = val
                return clean_values
            else:
                # For Postgres (using %s), return sanitized list/tuple
                clean_values_list = []
                for raw_value in values:
                    if isinstance(raw_value, str):
                        clean_values_list.append(re.sub(clean_regex, "", raw_value))
                    elif isinstance(raw_value, datetime):
                        clean_values_list.append(raw_value.timestamp())
                    else:
                        clean_values_list.append(raw_value)
                return type(values)(clean_values_list)
